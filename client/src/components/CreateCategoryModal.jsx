...
const CreateCategoryModal = ({categories, setCategories, setAddCategoryModal}) => {
    ...
    const createCategoryHandler = async (e) => {
        e.preventDefault()
        try {
            const {data: res} = await addCategory({categoryName})
  ...
  return (
    <div className="absolute top-0 left-0 h-screen w-full bg-black/90 z-10 flex items-center justify-center">
      <div className=" bg-white w-[50%] rounded">
        <div className="p-4 ">
          <form className="flex flex-col" onSubmit={createCategoryHandler}>
