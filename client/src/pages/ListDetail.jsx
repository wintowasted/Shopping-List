...
const ListDetail = () => {
  ...
  const addProductHandler = async (product) => {
    ...
    try {
      ...
      const {data: result} = await addProductToList(list.listId, product.productId);
      console.log("Add to list", result)
  ...
  return (
    <>
        <div className="w-full flex items-center justify-center">
          ...
          <button className={list.isLocked ? "bg-purple-600 text-white rounded-3xl h-10 px-3 text-sm font-semibold" : "bg-green-300 rounded-3xl h-10 px-3 text-sm font-semibold"} onClick={handleLockList}>{list.isLocked ? "Complete" : "Go Shopping" }</button>
          </div>
            <div className=" shadow-lg  mx-auto p-4 rounded pb-4 hover:shadow-2xl border-2 bg-white  ">
                ...
                <div className="cursor-pointer float-right" onClick={() => navigate(-1)}>
                  <AiOutlineClose />
                </div>
            
              <div
                ref={productMenuRef}
                className={list.isLocked ? "hidden": "wfull bg-green-400 flex items-center px-4 gap-x-3 mt-8 py-2 cursor-pointer rounded-2xl relative"}
                    ...
                    <div className="w-full flex justify-between items-center bg-white mt-2 rounded-md px-8 py-4">
                      <div className="flex items-center gap-x-6">
                        ...
                        <div className="flex flex-col justify-center items-center">
                          <label className={product.isChecked ?"text-xl text-gray-600/80 line-through" : "text-xl"} htmlFor="checkProduct">
                            {product.productName}
                          </label>
                          {product.description && (
                        ...
                        <div
                          className={list.isLocked ? "hidden": "text-xl cursor-pointer ml-4"}
                          onClick={() => {
                      ...
                      <div className="flex items-center gap-x-6 text-xl">
                        ...
                        <div className="cursor-pointer h-[35px] w-[35px]">
                          <img className="h-full object-contain w-full" src={`data:image/svg+xml;base64,${product.productImage}`} alt="product" />
                        </div>
                        <div
                          className={list.isLocked ? "hidden" : "cursor-pointer hover:text-red-600"}
